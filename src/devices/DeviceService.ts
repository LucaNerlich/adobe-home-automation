...
import {getTopicValue, STRATEGIES_FORM_NAME, TOPIC_FORM_NAME} from './FormService'
 ...
 */
function createConsumer(topic: string, strategyType: StrategyType): Consumer | null {
    let result: Consumer | null = null

    if (strategyType != null) {
        switch (strategyType) {
            case StrategyType.BOOLEAN_STRATEGY.valueOf():
                result = new ConsumerImpl(topic, 'some-label', new BooleanStrategy())
                break
            case StrategyType.NUMBER_STRATEGY.valueOf():
                result = new ConsumerImpl(topic, 'some-label', new NumberStrategy())
                break
            default:
                console.error('Cannot create provider for non existent strategy: ', strategyType)
                break
...
export function createDeviceWithFormData(formData: FormData, deviceType: DeviceType) {
    ...
    formData.forEach((value, key) => {
        if (key === TOPIC_FORM_NAME) {
            topic = getTopicValue(value.toString())
    ...
    if ((topic && topic.length > 0) && strategyType) {
        switch (deviceType) {
            ...
            case DeviceType.CONSUMER:
                const consumerElement = createConsumer(topic, strategyType)?.getElement()
                if (consumerElement) {
